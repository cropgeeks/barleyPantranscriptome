...
#This is the script for pass2 - pass1 is in script STAR_pass1.sh
...
#for pass 2 we use the regenerated index with the consolidated, filtered splice junctions from pass 1 (from across all our samples)
starIndex=refseq/STAR_index_pass2
...
do	
	...
	do
		...
		outFileNamePrefix=$sampleName"_"$tissue""$biorep"_pass2_"
		...
		--outSAMattrRGline ID:$sampleName""$tissue""$biorep SM:$sampleName LB:$tissue""$biorep PL:Illumina \
		--twopassMode None \
		--alignIntronMin 60 \
		...
		samtools index $output_dir/$sampleName"_"$tissue""$biorep"_pass2_"Aligned.sortedByCoord.out.bam
